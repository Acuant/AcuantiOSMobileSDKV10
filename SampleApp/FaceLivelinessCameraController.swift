...
class FaceLivelinessCameraController : UIViewController,LiveFaceCaptureDelegate{
    ...
    func startCameraView() {
        ...
        captureSession = Controller.getFaceCaptureSession(delegate: self,captureDevice: captureDevice,previewSize:self.view.layer.bounds.size)
        ...
        if(liveFaceDetails?.faceRect != nil && liveFaceDetails?.image != nil){
            ...
            if(liveFaceDetails?.isLiveFace)!{
                if(self.captured == false){
                    ...
                    rootVC.capturedLiveFace = liveFaceDetails?.image
                    self.navigationController?.popViewController(animated: true)
                    rootVC.liveFaceCaptured(image: (liveFaceDetails?.image)!)
                }
            }
    ...
    override func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator) {
        ...
        coordinator.animate(alongsideTransition: { (UIViewControllerTransitionCoordinatorContext) -> Void in
            ...
            self.captureSession = Controller.getFaceCaptureSession(delegate: self,captureDevice: captureDevice,previewSize:self.view.layer.bounds.size)
